...
export class ReturnListService {
  ...
  refreshReturn(rtn: IReturn): void {
    this._returns.update(returns => returns.map(r => (r.id === rtn.id ? { ...r, ...rtn } : r)));
  }
  removeReturn(returnId: number): void {
    this._returns.update(returns => returns.filter(r => r.id !== returnId));
  }
}
